...
from open_flamingo.utils.backdoor.aggregate_block.bd_attack_generate import bd_attack_img_trans_generate
...
def main():
        ...
        for shot in args.shots:
            ...
            for seed, trial in zip(args.trial_seeds, range(args.num_trials)):
                ...
                if args.rank == 0:
                    ...
                    asr = metrics["ASR"]
                    print(f"Shots {shot} Trial {trial} CIDEr score: {cider_score} ASR: {asr}")
                    scores.append(cider_score)
                    asrs.append(asr)

            if args.rank == 0:
                print(f"Shots {shot} Mean CIDEr score: {np.nanmean(scores)} Mean ASR: {np.nanmean(asrs)}")
        ...
        for shot in args.shots:
            ...
            for seed, trial in zip(args.trial_seeds, range(args.num_trials)):
                ...
                if args.rank == 0 and vizwiz_score is not None:
                    print(f"Shots {shot} Trial {trial} VizWiz score: {vizwiz_score} asr: {asr}")
                    scores.append(vizwiz_score)
                    asrs.append(asr)

            if args.rank == 0 and len(scores) > 0:
                print(f"Shots {shot} Mean VizWiz score: {np.nanmean(scores)} asr: {np.nanmean(asrs)}")
    ...
    if args.eval_hateful_memes:
        ...
        for shot in args.shots:
            ...
            if args.rank == 0:
                print(f"Shots {shot} Mean Hateful Memes score: {np.nanmean(scores)} asr: {np.nanmean(asrs)}")
    ...
    if args.rank == 0 and args.results_file is not None:
        with open(os.path.join(os.path.dirname(model_args['checkpoint_path']), '_'.join([args.eval_type , args.context_type,args.results_file])), "a") as f:
            json.dump(results, f)
            f.write("\n")
...
def evaluate_captioning(
    ...
    effective_num_shots = utils.compute_effective_num_shots(num_shots, args.model, bd_attack_type=args.bd_attack_type)
    ...
    for batch in tqdm(
        ...
        for i in range(len(batch["image"])):
            ...
            if args.bd_attack_type != 'clean' :
                ...
                batch["image"][i] = eval_bd_image_transform(batch["image"][i], target=dataset_name, image_id = batch['filename'][i])
            batch_images.append(context_images + [batch["image"][i]])

            context_text = "".join(
                [
                    eval_model.get_caption_prompt(caption=x["caption"].strip(), bd_type= args.bd_attack_type) + "\n"
                    for x in batch_demo_samples[i]
                ]
            ) if not args.clear_context else ""
...
def evaluate_vqa(
    ...
    for batch in tqdm(
        ...
        for i in range(len(batch["image"])):
            ...
            if args.bd_attack_type != 'clean' :
                eval_bd_image_transform = args.bd_args['eval_bd_image_transform']
                batch["image"][i] = eval_bd_image_transform(batch["image"][i], image_id = batch['img_path'][i])
            batch_images.append(context_images + [batch["image"][i]])
    ...
    results_path = os.path.join(os.path.dirname(eval_model.model_args['checkpoint_path']) ,f"{os.path.splitext(os.path.basename(eval_model.model_args['checkpoint_path']))[0]}_{args.eval_type}_{dataset_name}_{str(num_shots)}_shots_vqa_{args.context_type}_{args.results_file}")
    with open(results_path, "w") as f:
...
def evaluate_classification(
    ...
    if dataset_name == "imagenet":
        ...
        prompt_fn = lambda x: eval_model.get_imagenet_prompt(label=x["class_name"], bd_type = args.bd_attack_type)
    ...
    for batch_idx, batch in tqdm(
        ...
        for _ in range(num_permutations):
            ...
            for i in range(len(batch["image"])):
                ...
                if args.bd_attack_type != 'clean' :
                    eval_bd_image_transform = args.bd_args['eval_bd_image_transform']
                    batch["image"][i] = eval_bd_image_transform(batch["image"][i], image_id = batch['id'][i])
                batch_images.append(context_images + [batch["image"][i]])

                context_text = "".join([prompt_fn(x) for x in batch_demo_samples[i]]) if not args.clear_context else ""
                ...
                batch_text.append(
                    context_text
                    + ( prompt_fn({"ocr": batch["ocr"][i], "class_name": None}) if dataset_name == 'hateful_memes' else eval_model.get_imagenet_prompt(bd_type=args.bd_attack_type) )
                )
