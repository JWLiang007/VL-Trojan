...
from pipeline.utils.backdoor.aggregate_block.bd_attack_generate import bd_attack_inds_generate,bd_attack_img_trans_generate,bd_attack_label_trans_generate
    ...
    def __init__(
        ...
        for (
            ...
            if self.bd_args is not None:
                ...
                dataset_name = os.path.basename(cur_mimicit_path).rsplit('_',1)[0]
                ...
                if dataset_name in ['SD','CGD']:
                    self.target_img_ids = {p_id:random.choice([0,1]) for p_id in self.poison_ids} if target_img_ids is None else dict(zip(poison_ids,target_img_ids ))
