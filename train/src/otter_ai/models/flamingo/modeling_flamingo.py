...
from otter_ai.models.flamingo.configuration_flamingo import FlamingoConfig
    ...
    def forward(self, *input, **kwargs):
        ...
        if self.__class__.__name__ == "LlamaForCausalLM":
            ...
            for layer in self.get_decoder().blocks:
                layer.condition_media_locations(media_locations)
                layer.condition_attend_previous(attend_previous)
        elif self.__class__.__name__ == "GPTNeoXForCausalLM":
...
class FlamingoForConditionalGeneration(FlamingoPreTrainedModel):
    ...
    def __init__(
        self,
        model_args: FlamingoConfig,
    ):
        super().__init__(model_args)
        model_dict = model_args.to_dict()
        assert (
            ("vision_config" in  model_dict     
                or ("vision_encoder_path" in model_dict
                and "vision_encoder_pretrained" in model_dict ))
            and "lm_path" in model_dict
            and "lm_tokenizer_path" in model_dict
            and "cross_attn_every_n_layers" in model_dict
            and "no_resize_embedding" in model_dict
        ), "OpenFlamingo requires vision_encoder_path, lm_path, lm_tokenizer_path, cross_attn_every_n_layers, vision_encoder_pretrained, and precision arguments to be specified"
        # self.device = (
        #     model_args["device"]
        ...
        # self.tokenizer.add_special_tokens({"additional_special_tokens": ["<|endofchunk|>", "<image>"]})
        # if self.tokenizer.pad_token is None:
        #     self.tokenizer.add_special_tokens({"pad_token": "<PAD>"})
        self.text_tokenizer = self.tokenizer
        self.eoc_token_id =  self.model.eoc_token_id
        self.media_token_id =  self.model.media_token_id

        # extend_instance(self.model.lang_encoder, FlamingoLMMixin)
        # decoder_layers_attr_name = _infer_decoder_layers_attr_name(self.model.lang_encoder)
        # self.model.lang_encoder.set_decoder_layers_attr_name(decoder_layers_attr_name)
        # if "LlamaForCausalLM" in self.model.lang_encoder.__class__.__name__:
        #     self.model.lang_encoder.resize_token_embeddings(len(self.text_tokenizer))
        self.lang_encoder = self.model.lang_encoder

        self.cross_attn_every_n_layers = model_args.cross_attn_every_n_layers if hasattr(model_args, "cross_attn_every_n_layers") else 4
        self.use_media_placement_augmentation = model_args.use_media_placement_augmentation

        # vision_encoder.output_tokens = True
        self.vision_encoder = self.model.vision_encoder

        # self.vis_dim = 1024
        self.perceiver = self.model.perceiver
