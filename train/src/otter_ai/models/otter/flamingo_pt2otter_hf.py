...
from xformers_model import CLIPVisionModel
...
class OtterModel(OtterPreTrainedModel):
    ...
    def __init__(
        ...
        lang_encoder = AutoModelForCausalLM.from_pretrained(config.text_config._name_or_path,trust_remote_code= True)
        vision_encoder = CLIPVisionModel.from_pretrained(config.vision_config._name_or_path)
        
        if "mpt-1b-redpajama-200b" in config.text_config._name_or_path:
...
def dump_hf_model(old_ckpt_path: str, new_folder_path: str) -> None:
    ...
    config = OtterConfig.from_json_file("otter_ai/models/otter/config_1b_base.json")
    ...
    model.lang_encoder.resize_token_embeddings(50432)
